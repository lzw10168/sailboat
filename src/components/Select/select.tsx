...
export const Select: FC<SelectProps> = props => {
  ...
  const containerClass = classNames('sailboat-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="sailboat-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition in={menuOpen} animation="zoom-in-top" timeout={300}>
          <ul className="sailboat-select-dropdown">{generateOptions()}</ul>
        </Transition>
      </SelectContext.Provider>
      {multiple && (
        <div
          className="sailboat-selected-tags"
          style={{ maxWidth: containerWidth.current - 32 }}>
          {selectedValues.map((value, index) => {
            return (
              <span className="sailboat-tag" key={`tag-${index}`}>
                {value}
                <Icon
                  className="sailboat-tag_icon"
                  icon="times"
                  onClick={() => {
